...
//!
//! aurestream-core runs inside the AureStreamTunService (an SCM service process),
...
pub(crate) fn spawn(app: AppHandle, process_mode: Arc<ProxyMode>, spawn_epoch: u64) {
    tokio::spawn(async move {
        use aurestream_plugin_tun::scm::{query_state, QueriedState};
        ...
        loop {
            let still_tun = {
                ...
                m.mode
                    .as_ref()
                    .map(|x| matches!(**x, ProxyMode::IntoProxy))
                    .unwrap_or(false)
            };
            if !still_tun {
                return;
