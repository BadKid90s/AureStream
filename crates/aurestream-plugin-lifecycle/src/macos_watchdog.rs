...
use crate::monitor::handle_process_termination;
use crate::process::ProcessManager;
use crate::ProxyMode;
use crate::state_machine::{transition, EngineStateCell, Intent};
use crate::EVENT_STATUS_CHANGED;
use crate::readiness;
...
async fn restart_tun_send_safe(app: AppHandle, path: Arc<String>) -> Result<(), String> {
    ...
    let _pid = tokio::task::spawn_blocking(move || aurestream_plugin_tun::macos::start_tun_via_helper(&path_c, &crate::log::resolve_singbox_log_path(&app_c).map(|p| p.to_string_lossy().to_string()).unwrap_or_default(), false, &crate::helper::extract_tun_gateway_from_config(&path_c).unwrap_or_default()))
        .await
