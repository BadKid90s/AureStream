...
fn run(cmd: Commands) -> aurestream_plugin_proxy::Result<()> {
    match cmd {
        Commands::Query => {
        ...
        Commands::Set {
            ...
            if !(1..=15).contains(&flags) {
                return Err(aurestream_plugin_proxy::Error::ParseStr(format!(
        ...
        #[cfg(target_os = "windows")]
        Commands::UwpGet => {
            let list = aurestream_plugin_proxy::AppContainer::get_exemption()?;
        ...
        Commands::UwpSet => {
            ...
            let sids: Vec<String> = entries
                ...
                .map(|c| c.sid)
                .collect();
            let updated = aurestream_plugin_proxy::AppContainer::set_exemption(&sids)?;
...
fn parse_server(server: &str) -> aurestream_plugin_proxy::Result<(String, u16)> {
    if let Some((host, port_str)) = server.rsplit_once(':') {
        let port: u16 = port_str
            .parse()
            .map_err(|_| aurestream_plugin_proxy::Error::ParseStr(server.into()))?;
        Ok((host.to_string(), port))
    } else {
        Err(aurestream_plugin_proxy::Error::ParseStr(format!(
