      ...
      <form onSubmit={(e) => { e.preventDefault(); navigate("/dashboard") }} className="flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between mt-1">
          <label className="flex items-center gap-2 cursor-pointer group" onClick={() => setRemember(!remember)}>
            <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all ${remember ? 'bg-primary border-primary text-white' : 'border-border-glass bg-surface-active group-hover:border-primary/50 text-transparent'}`}>
              <I.Check />
            </div>
