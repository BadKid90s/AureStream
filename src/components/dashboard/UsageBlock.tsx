...
function buildSmoothPath(values: readonly number[], width: number, height: number): string {
...
function buildSmoothLine(values: readonly number[], width: number, height: number): string {
  const n = values.length
  if (n < 2) return ''
...
export function UsageBlock({
  ...
  const total = hasVolume ? (uploadTotal ?? 0) + (downloadTotal ?? 0) : undefined
  ...
  const noData = uploadSeries.every((v) => v === 0) && downloadSeries.every((v) => v === 0)
    ...
    <div className={cn('flex flex-col gap-4', className)}>
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex flex-col gap-0.5">
          <span className="text-sm font-semibold text-foreground">用量信息</span>
          <span className="text-[10px] leading-tight text-muted-foreground">内核本会话累计（非订阅账单）</span>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-muted/25 px-2 py-3">
        ...
        >
          <defs>
            <linearGradient id={`${CHART_GRAD_ID}-down`} x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="var(--color-primary)" stopOpacity="0.25" />
              <stop offset="100%" stopColor="var(--color-primary)" stopOpacity="0.02" />
            </linearGradient>
            <linearGradient id={`${CHART_GRAD_ID}-up`} x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="var(--color-emerald-500)" stopOpacity="0.22" />
              <stop offset="100%" stopColor="var(--color-emerald-500)" stopOpacity="0.01" />
            </linearGradient>
          </defs>
          <rect width={VIEW_W} height={VIEW_H} className="fill-transparent" />
          {noData ? (
            <line
              x1={PAD_X} y1={VIEW_H / 2} x2={VIEW_W - PAD_X} y2={VIEW_H / 2}
            ...
            <>
              {/* 下载面积 + 描边 */}
              {downloadArea ? (
                <path d={downloadArea} fill={`url(#${CHART_GRAD_ID}-down)`} stroke="none" />
              ) : null}
              {downloadLine ? (
                <path d={downloadLine} fill="none" className="stroke-primary [vector-effect:non-scaling-stroke]" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              ) : null}
              {/* 上传面积 + 描边 */}
              {uploadArea ? (
                <path d={uploadArea} fill={`url(#${CHART_GRAD_ID}-up)`} stroke="none" />
              ) : null}
              {uploadLine ? (
                <path d={uploadLine} fill="none" className="stroke-emerald-500 [vector-effect:non-scaling-stroke]" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              ) : null}
            </>
          )}
        </svg>
