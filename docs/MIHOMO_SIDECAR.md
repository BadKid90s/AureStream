...
在 **macOS** 上，修改系统代理**一律**通过 **`osascript` + `with administrator privileges`** 以**管理员身份**批量执行 `networksetup`，连接与断开各会**至多弹一次**系统密码框。

断开或应用退出并正常结束 sidecar 时，会尝试将上述代理**全部关闭**。若清除失败，终端会打印 `[system-proxy]` 日志，需用户在 **系统设置 › 网络 › 详情 › 代理** 中手动恢复。
